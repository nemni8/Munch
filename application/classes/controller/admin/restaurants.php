...
function restaurant_search() {
    $restaurants = DB::select()
			            ->from('restaurants')
			            ->where('active','=',1);
    $this->template->content = View::factory('admin/restaurants/search')
			->set('post', $_POST)
        ...
        if ($_POST)
        {
            //if (isset($_POST['rest_name'])) {     MAYBE KOBI WILL CREATE AUTOCOMPLETE!!!!
              //      $restaurants->and_where('name','like','%'.$_POST['rest_name'].'%');
            //}
            if (isset($_POST['city_id'])) {
                    $restaurants->and_where('city_id','=',$_POST['city_id']);
            }
            if (isset($_POST['kitchen_type'])) {
                    $temp= clone $restaurants;
                    $restaurants= DB::select('rest.*','categories_restaurants.category_id')
            ...
            if (isset($_POST['min_order'])) {
                    $restaurants->and_where('delivery_min','<=',$_POST['min_order']);
            }
            if (isset($_POST['delivery_cost'])) {
                    $restaurants->and_where('delivery_cost','<=',$_POST['delivery_cost']);
            }
            if (isset($_POST['delivery_time'])) {
                    $restaurants->and_where('delivery_time','<=',$_POST['delivery_time']);
            }
            if (isset($_POST['kosher_level'])) {
                    $restaurants->and_where('kosher_type','=',$_POST['city_id']);
            }
            $restaurants->execute();
